def sub(template, ns, max_iter=50):
    ...
    def template_substitute(template, ns, substitute_handler):
        ...
        return re.sub(r'(?s)(?<![$])(?:\${{(.+?)}}|\${(.+?)}|\$([a-zA-Z_]\w*))', lambda m: substitute_handler(ns, m.group(1) or m.group(2) or m.group(3), m.group(0)), template)
    def format_ret(x, orig):
        if (type(x) == list or type(x) == tuple) and all(type(x) == str for x in x):
            return ' '.join(x)
        if x == None or type(x) != str and type(x) != int and type(x) != float:
            return orig
        else:
            return str(x)
    old, cur = "", template
