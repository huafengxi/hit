...
** user init file
config file has the form =config[0-9]?.py=, searched in =pwd= and =_hit_path_=, the single =[0-9]= digit is interpreted as version number,
the largest versioned file will be regarded as config file.
: cfg=c.py ./hit.py <cmd...>         # use c.py as alternative config file.
